...
function workspaceRelativePath(workspaceRoot, targetPath) {
  ...
  const relative2 = path.isAbsolute(trimmed) ? path.relative(workspaceRoot, trimmed) : trimmed;
  const normalized = path.posix.normalize(toPosixPath(relative2));
  if (normalized === "." || normalized === "./") return ".";
  if (normalized === ".." || normalized.startsWith("../")) return void 0;
  return normalized;
}
function notesDirectory(workspaceRoot, scope) {
...
async function readAllNotes(workspaceRoot) {
...
async function readNotesFrom(directory, scope) {
  ...
  const notes = await Promise.all(
    fileNames.map(async (fileName) => {
      ...
      try {
        return parseNote(await import_node_fs.promises.readFile(notePath, "utf8"), notePath, scope);
      } catch {
        return void 0;
      }
    })
...
async function listNoteFileNames(directory) {
  try {
    const entries = await import_node_fs.promises.readdir(directory);
    return entries.filter((entry) => entry.endsWith(".md"));
  } catch {
    return [];
  }
}
function isDirectoryNote(note) {
  return note.targetPath === "." || note.targetPath.endsWith("/");
}
function workspaceNotes(notes) {
...
async function main() {
  ...
  const context = event === "session" ? describeRepositoryNotes(notes) : describeFileNotes(notes, workspaceRoot, payload.tool_input?.file_path);
  if (context) emitContext(event, context);
}
function describeRepositoryNotes(notes) {
  const broadNotes = workspaceNotes(notes);
  if (broadNotes.length === 0) return void 0;
  return [
...
function describeFileNotes(notes, workspaceRoot, filePath) {
...
function renderNote(note) {
  const target = isDirectoryNote(note) ? note.targetPath === "." ? "whole repository" : note.targetPath : [note.targetPath, note.symbol, note.line === void 0 ? void 0 : `line ${note.line}`].filter(Boolean).join(" \xB7 ");
  return [`--- ${target} [${note.type}]`, note.body, ""].join("\n");
}
function emitContext(event, additionalContext) {
